...
version$version.string # R version 4.4.2
...
prepare_pisa_data <- function(data, countries, predictors, convert_to_na = TRUE) {
  ...
  # Replicate weights
  weight_cols <- grep("^W_FSTURWT[0-9]+$", names(data), value = TRUE) # Replicate weight columns
  # Subset data: Retain only selected countries and relevant variables
  essential_vars <- c("STRATUM", "CNTSCHID", "W_FSTUWT")
  all_vars <- unique(c(essential_vars, predictors))
  data_subset <- data %>%
    filter(CNT %in% countries) %>% # Keep only specified countries
    select(CNT, all_of(all_vars), all_of(weight_cols), all_of(pv_cols)) # Retain key variables
  # Remove SPSS-style labels
  data_subset <- data_subset %>%
    mutate(across(everything(), haven::zap_labels)) 
  # Function to handle different types of missing values with type checking
  recode_missing <- function(x) {
    # Ensure x is numeric - if not, convert with warning suppression
    if(!is.numeric(x)) {
    ...
    if(convert_to_na) {
      ...
      result <- x  
      for(code_list in invalid_codes) {
        # Replace each type of invalid code with NA
        result[!is.na(x) & x %in% code_list] <- NA_real_
      }
      return(result)
    } else {
      return(x)  # Return as-is if not converting to NA
    }
  }
...
# Function to remove strata with less than 2 PSUs
remove_single_psu_strata <- function(data) {
  # Count PSUs per stratum
  psu_counts <- data %>%
    group_by(CNT, STRATUM) %>%
    summarise(psu_count = n_distinct(CNTSCHID), .groups = "drop")
  # Identify strata with at least 2 PSUs
  valid_strata <- psu_counts %>%
    filter(psu_count >= 2)
  # Filter data to keep only valid strata
  data_filtered <- data %>%
    inner_join(valid_strata, by = c("CNT", "STRATUM"))
  # Report how many observations were removed
  message("Removed ", nrow(data) - nrow(data_filtered), 
          " rows from strata with less than 2 PSUs")
  message("Final dataset has ", nrow(data_filtered), " rows")
...
analyze_weighted_predictors <- function(data, predictors) {
  ...
  rep_weights <- grep("^W_FSTURWT[0-9]+$", names(data), value = TRUE)
  ## Step 1: Missing data analysis
  missing_analysis <- lapply(predictors, function(pred) {
    # Define survey design for complex weighting
    design <- tryCatch({
      svydesign(
        ids = ~CNTSCHID,
        ...
        data = data,
        nest = TRUE,
        repweights = "W_FSTURWT[0-9]+",  # Replicate weights
      ...
      warning("Error creating survey design: ", e$message)
      # Fallback to basic design if replicate weights cause issues
      svydesign(
        ids = ~CNTSCHID,
        weights = ~W_FSTUWT,
        strata = ~STRATUM,
    ...
    # Weighted proportion of missing values for each predictor
  ...
  pv_correlations <- lapply(predictors, function(pred) {
    # Add error handling for when a predictor is all NA
    ...
    pv_cors <- sapply(1:10, function(i) {
      ...
      # Error handling for correlation calculation
      tryCatch({
        ...
        warning("Error calculating correlation for ", pred, " and ", pv_col, ": ", e$message)
        return(NA_real_)
      })
    })
  ...
  rownames(pred_cors) <- colnames(pred_cors) <- predictors 
  # Pairwise correlation calculation
  for(i in 1:length(predictors)) {
    for(j in i:length(predictors)) {
      if(i != j) {
        # Check if either predictor is all NA
        if(all(is.na(data[[predictors[i]]])) || all(is.na(data[[predictors[j]]]))) {
          pred_cors[i,j] <- pred_cors[j,i] <- NA_real_
          next
        }
        ...
        cor_est <- tryCatch({
          ...
          warning("Error calculating correlation between ", 
                  predictors[i], " and ", predictors[j], ": ", e$message)
          return(NA_real_)
        })
...
# Weighted correlation function with error handling
weighted_cor <- function(x, y, weights) {
  # Remove NAs
  complete_cases <- !is.na(x) & !is.na(y) & !is.na(weights)
  x <- x[complete_cases]
  y <- y[complete_cases]
...
# Function to filter through variables based on criteria
suggest_predictors <- function(analysis_results, 
  ...
  high_cors <- analysis_results$predictor_correlations %>%
    filter(abs(r) > correlation_threshold & Parameter1 != Parameter2 & !is.na(r))
  ...
  if(nrow(high_cors) > 0) {
    for(i in 1:nrow(high_cors)) {
      ...
      # Skip if either predictor has already been removed
      if(!(pred1 %in% keep_predictors) || !(pred2 %in% keep_predictors)) {
        next
      }
  ...
  final_predictors <- analysis_results$pv_correlations %>%
    filter(predictor %in% keep_predictors,
           ...
           !is.na(avg_correlation)) %>%  # Exclude NA correlations
    pull(predictor)
  return(final_predictors)
}

# Remove strata with only one PSU before analysis
data_subset <- remove_single_psu_strata(data_subset) 
...
# Get suggested predictors 
final_predictors <- suggest_predictors(
  results,
  missing_threshold = 30, # Remove if more than 30% missing 
  correlation_threshold = 0.8, # Remove one predictor from pairs with correlation > 0.8
  min_pv_correlation = 0.1 # Keep if at least 0.1 correlation with PV
)
...
# Create item values
ST267_vars <- paste0("ST267Q", sprintf("%02d", 1:08), "JA") # RELATST: Quality of student-teacher relationship
ST263_vars <- paste0("ST263Q", sprintf("%02d", seq(2, 8, by = 2)), "JA") # GROSAGR: Growth mindset
ST273_vars <- paste0("ST273Q", sprintf("%02d", 1:07), "JA") # DISCLIM: Disciplinary climate
ST290_vars <- paste0("ST290Q", sprintf("%02d", 1:09), "WA") # MATHEFF: Self-efficacy: Formal and applied
ST291_vars <- paste0("ST291Q", sprintf("%02d", 1:10), "JA") # MATHEFF21: Self-efficacy: reasoning and 21st century 
ST293_vars <- paste0("ST293Q", sprintf("%02d", 1:09), "JA") # MATHPERS: Proactive mathematics study behaviour
ST292_vars <- paste0("ST292Q", sprintf("%02d", 1:06), "JA") # ANXMAT: Mathematics anxiety
...
analyze_missing_data_survey <- function(data, vars, weights, strata_var, psu_var) {
  ...
  ## Step 6: Loop through all variables
  for(var in vars) {
    ...
    for(exp_var in explanatory) {
      ...
      tryCatch({
        ...
        resp_mean <- svymean(form, 
                             subset(survey_design, get(paste0(var, "_missing")) == 0), 
                             na.rm = TRUE)
        # Calculate the mean for missing values
        nonresp_mean <- svymean(form, 
                                subset(survey_design, get(paste0(var, "_missing")) == 1), 
                                na.rm = TRUE)
        ...
        results[[exp_var]] <- data.frame(
          ...
          p = round(test$p.value, 4) # p-value
        )
      }, error = function(e) {
...
target_vars <- c("ANXMAT", "MATHPERS", "MATHEFF", "MATHEF21",
                 "DISCLIM", "GROSAGR", "RELATST", "IMMMIG",
                 "ESCS") # Remove MATHEF21, ESCS, and IMMIG 
...
identify_auxiliary_vars <- function(data, target_vars, threshold_cor = 0.5, 
  ...
  potential_auxiliaries <- names(missing_prop)[missing_prop <= threshold_miss & 
                                                 ...
                                                 !(names(missing_prop) %in% pv_vars)]  # Exclude PVs
...
create_pv_datasets <- function(data, target_vars, survey_vars, auxiliary_vars, math_pv_vars) {
  ...
  # Determine how many sets of PVs there are (10 for PISA 2022)
  ...
  # Create a list to store my datasets
  pv_datasets <- list()
...
# Function to reverse code Mathematics Anxiety
reverse_code <- function(data) {
  # Reverse code ANXMAT (higher values = lower anxiety)
...
# Extract and combine all important variable means for figures
  ...
  for (country in countries) {
    ...
    for (pv in 1:10) {
      for (imp in 1:10) {
        ...
        full_model <- svyglm(
          MATH_PV ~ ANXMAT + MATHPERS + MATHEFF + 
            DISCLIM + GROSAGR + RELATST, # + ESCS + MATHEF21 + IMMIG, 
          design = design,
          family = gaussian()
        )
        # Store models
...
# Function to retrieve outcome variable mean
